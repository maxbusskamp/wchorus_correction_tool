...
def create_simpson():  # Write simpson input files
    ...
    proc pulseq {} {
        ...
        if {[string equal $par(type) "double_echo"] || [string equal $par(type) "WCHORUS"]} {
            ...
            pulse_shaped $par(tw2) $rfsh2
            delay $par(tau1)
        } elseif {[string equal $par(type) "WURSTsingle"]} {
            pulse_shaped $par(tw1) $rfsh1
        } elseif {[string equal $par(type) "create_shapes_WURSTsingle"] || [string equal $par(type) "create_shapes_WCHORUS"] || [string equal $par(type) "create_shapes_double_echo"] || [string equal $par(type) "create_shapes_ABSTRUSE"]} {
        } else {
    ...
    proc main {} {
        ...
        if { $argc != 22 } {
            ...
            set par(Delta1)                     [lindex $argv 5]
            set par(Delta2)                     [lindex $argv 6]
            set par(Delta3)                     [lindex $argv 7]
            set par(N1)                         [lindex $argv 8]
            set par(N2)                         [lindex $argv 9]
            set par(N3)                         [lindex $argv 10]
            set par(rf_factor1)                 [lindex $argv 11]
            set par(rf_factor2)                 [lindex $argv 12]
            set par(rf_factor3)                 [lindex $argv 13]
            set par(tau1)                       [lindex $argv 14]
            set par(tau2)                       [lindex $argv 15]
            set par(tau3)                       [lindex $argv 16]
            set par(filename_phasecorrect)      [lindex $argv 17]
            set par(ss_offset)                  [lindex $argv 18]
            set par(beta_hs)                    [lindex $argv 19]
            set par(zeta)                       [lindex $argv 20]
            set par(tan_kapp)                   [lindex $argv 21]
        }
        ...
        if {[string equal $par(type) "create_shapes_WURSTsingle"] || [string equal $par(type) "create_shapes_WCHORUS"] || [string equal $par(type) "create_shapes_double_echo"] || [string equal $par(type) "create_shapes_ABSTRUSE"]} {
    ...
    for {set index 0} {$index<$par(phasecycles)} {incr index} {
        ...
        if {[string equal $par(type) "double_echo"] || [string equal $par(type) "WCHORUS"]} {
            ...
            set rfsh1 [list2shape [wurst $par(N1) $par(tw1) $par(Delta1) $par(rf1) $par(ph1) $par(stepsize)]]
            ...
            set rfsh2 [list2shape [wurst $par(N2) $par(tw2) $par(Delta2) $par(rf2) $par(ph2) $par(stepsize)]]
            ...
            set rfsh3 [list2shape [wurst $par(N3) $par(tw3) $par(Delta3) $par(rf3) $par(ph3) $par(stepsize)]]

            # Set filenames for WURST type experiments
            set par(filename) $par(name)_rffactor_$par(rf_factor1)_$par(rf_factor2)_$par(rf_factor3)_tw_$par(tw1)_$par(tw2)_$par(tw3)_delays_$par(tau1)_$par(tau2)_$par(tau3)_delta_$par(Delta1)_$par(Delta2)_$par(Delta3)_N_$par(N1)_$par(N2)_$par(N3)
        } elseif {[string equal $par(type) "create_shapes_WCHORUS"]} {
            # Set first WURST pulse (excitation)
            set par(sweep_rate1) [expr ($par(Delta1)*1e3)/($par(tw1)*1e-6)]
            set par(rf1) [format "%.2f" [expr $par(rf_factor1)*sqrt($par(sweep_rate1))]]
            set rfsh1 [list2shape [wurst_phasecorr $par(N1) $par(tw1) $par(Delta1) $par(rf1) $par(filename_phasecorrect) 0.0]]

            # Set second WURST pulse (refocussing)
            set par(sweep_rate2) [expr ($par(Delta2)*1e3)/($par(tw2)*1e-6)]
            set par(rf2) [format "%.2f" [expr $par(rf_factor2)*sqrt($par(sweep_rate2))]]
            set rfsh2 [list2shape [wurst $par(N2) $par(tw2) $par(Delta2) $par(rf2) 0.0]]

            # Set third WURST pulse (refocussing)
            set par(sweep_rate3) [expr ($par(Delta3)*1e3)/($par(tw3)*1e-6)]
            set par(rf3) [format "%.2f" [expr $par(rf_factor3)*sqrt($par(sweep_rate3))]]
            set rfsh3 [list2shape [wurst $par(N3) $par(tw3) $par(Delta3) $par(rf3) 0.0]]

            # Set filenames for WURST type experiments
            set par(filename) $par(name)_$par(type)_rffactor_$par(rf_factor1)_$par(rf_factor2)_$par(rf_factor3)_tw_$par(tw1)_$par(tw2)_$par(tw3)_delays_$par(tau1)_$par(tau2)_$par(tau3)_delta_$par(Delta1)_$par(Delta2)_$par(Delta3)_N_$par(N1)_$par(N2)_$par(N3)

            set rfsh_shape1 [wurst_phasecorr $par(N1) $par(tw1) $par(Delta1) 100 $par(filename_phasecorrect)]
            set rfsh_shape2 [wurst $par(N2) $par(tw2) $par(Delta2) 100]
            set rfsh_shape3 [wurst $par(N3) $par(tw3) $par(Delta3) 100]

            printwave $rfsh_shape1 1
            printwave $rfsh_shape2 2
            printwave $rfsh_shape3 3

            set rfsh11 [list2shape [wurst_phasecorr $par(N1) $par(tw1) $par(Delta1) 1 $par(filename_phasecorrect)]]
            set rfsh22 [list2shape [wurst $par(N2) $par(tw2) $par(Delta2) 1]]
            set rfsh33 [list2shape [wurst $par(N3) $par(tw3) $par(Delta3) 1]]

            save_shape $rfsh1 $par(filename).simpson1
            save_shape $rfsh2 $par(filename).simpson2
            save_shape $rfsh3 $par(filename).simpson3
        } elseif {[string equal $par(type) "create_shapes_double_echo"]} {
            # Set first WURST pulse (excitation)
            set par(sweep_rate1) [expr ($par(Delta1)*1e3)/($par(tw1)*1e-6)]
            set par(rf1) [format "%.2f" [expr $par(rf_factor1)*sqrt($par(sweep_rate1))]]
            set rfsh1 [list2shape [wurst_phasecorr $par(N1) $par(tw1) $par(Delta1) $par(rf1) $par(filename_phasecorrect) 0.0]]
            ...
            # Set filenames for WURST type experiments
            set par(filename) $par(name)_$par(type)_rffactor_$par(rf_factor1)_$par(rf_factor2)_tw_$par(tw1)_$par(tw2)_delays_$par(tau1)_delta_$par(Delta1)_$par(Delta2)_N_$par(N1)_$par(N2)
            ...
            # Set filenames for WURST type experiments
            set par(filename) $par(name)_$par(type)_rffactor_$par(rf_factor1)_tw_$par(tw1)_delta_$par(Delta1)_N_$par(N1)
        } elseif {[string equal $par(type) "create_shapes_WURSTsingle"]} {
            # Set first WURST pulse (excitation)
            set par(sweep_rate1) [expr ($par(Delta1)*1e3)/($par(tw1)*1e-6)]
            set par(rf1) [format "%.2f" [expr $par(rf_factor1)*sqrt($par(sweep_rate1))]]
            set rfsh1 [list2shape [wurst_phasecorr $par(N1) $par(tw1) $par(Delta1) $par(rf1) $par(filename_phasecorrect) 0.0]]
    ...
    proc printwave {wave counter} {
        ...
        puts $fp "##TITLE= WURST-${par(N1)} shape (duration $par(tw1) us, sweep width $par(Delta1) kHz, step size: 0.05 us, sweep rate: $par(sweep_rate1) MHz/ms)"
        ...
        puts $fp "##DATE= "
        puts $fp "##TIME= "
        puts $fp "##\$SHAPE_PARAMETERS= Type: Wurst ; Total Sweep-Width \[Hz\] [expr $par(Delta1)*1000.0] ; Length of Pulse \[usec\] [expr 1.0*$par(tw1)] ; Amplitude Power Index ${par(N1)}.0 ; 1=High to low field, -1=Low to high field 1"
...
def exp_layout(exp_type, shape_type):  #Generate a WCHORUS Layout
    if(exp_type == 'WCHORUS'):  # define WCHORUS layout
        chorus_sequence = sequence_diagram(exp_type)
...
def selection_layout():  # Generates a selection Layout
    layout = [[sg.Text('Select Shape:'), sg.Combo(['WURST', 'tantanh'], key='shape_type')],
              [sg.Text('Select Pulse Sequence:'), sg.Combo(['WURSTsingle', 'double_echo', 'WCHORUS', 'ABSTRUSE'], key='exp_type')],
...
def read_parameter(exp_type, shape_type):  # Update the "output" text element to be the value of "input" element and set input to parameter
    ...
    if(exp_type == 'WCHORUS'):  # define WCHORUS parameter
        ...
        simpson_info = (f"Experiment = {exp_type} \n"
                    ...
                    f"N2 = {N2} \n"
                    f"N3 = {N3} \n")
    elif(exp_type == 'WURSTsingle'):  # define WCHORUS parameter
        simulation_window['delta1_out'].update(simulation_values['delta1'])
        simulation_window['tw1_out'].update(simulation_values['tw1'])
        simulation_window['rffactor1_out'].update(simulation_values['rffactor1'])
        simulation_window['N1_out'].update(simulation_values['N1'])

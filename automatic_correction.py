...
def create_simpson():  # Write simpson input files
    ...
    proc pulseq {} {
        ...
        if {[string equal $par(type) "double_echo"] || [string equal $par(type) "WCHORUS"]} {
            ...
            pulse_shaped $par(tw2) $rfsh2
            delay $par(tau1)
        } elseif {[string equal $par(type) "create_shapes_WCHORUS"] || [string equal $par(type) "create_shapes_double_echo"] || [string equal $par(type) "create_shapes_ABSTRUSE"]} {
        } else {
    ...
    proc main {} {
        ...
        if {[string equal $par(type) "create_shapes_WCHORUS"] || [string equal $par(type) "create_shapes_double_echo"] || [string equal $par(type) "create_shapes_ABSTRUSE"]} {
...
def simulate_sequence(exp_type):  # Start simulation with set parameter
    global delta
    global tw1
    global tw2
    global tw3
...
def selection_layout():  # Generates a selection Layout
    layout = [[sg.Text('Select Pulse Sequence')],
              [sg.Listbox(values=('WCHORUS', 'double_echo', 'ABSTRUSE'), size=(20, 12), key='exp_type', enable_events=True)],
              [sg.Button('Exit')]]
    return(layout)

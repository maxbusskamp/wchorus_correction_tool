...
def create_simpson():  # Write simpson input files
    ...
    proc pulseq {} {
        ...
        if {[string equal $par(type) "double_echo"] || [string equal $par(type) "CHORUS"] || [string equal $par(type) "loadshape_double_echo"]} {
            ...
            delay $par(tau3)
            acq
        } elseif {[string equal $par(type) "compressedCHORUS"]} {
    ...
    proc main {} {
        ...
        lappend ::auto_path ../../src/
            if {![namespace exists shapetools_liquid]} {
                ...
                namespace import shapetools_liquid::shape_add
                namespace import shapetools_liquid::zero_ampl
            }
    ...
    for {set index 0} {$index<$par(phasecycles)} {incr index} {
        ...
        } elseif {[string equal $par(type) "compressedCHORUS"] || [string equal $par(type) "compressedCHORUS_cycled"]} {
            # Set first WURST pulse (excitation)
            set par(sweep_rate1) [expr ($par(Delta1)*1e3)/($par(tw1)*1e-6)]
            set par(rf1) [format "%.2f" [expr $par(rf_factor1)*sqrt($par(sweep_rate1))]]
...
def sequence_diagram(exp_type):  # Generate base64 image of pulsesequence
    if(exp_type == 'CHORUS') or (exp_type == 'CHORUS_cycled') or (exp_type == 'compressedCHORUS_cycled'):
...
def exp_layout(exp_type, shape_type):  #Generate a CHORUS Layout
    ...
    if(exp_type == 'CHORUS') or (exp_type == 'CHORUS_cycled') or (exp_type == 'compressedCHORUS_cycled'):  # define CHORUS layout
...
def selection_layout():  # Generates a selection Layout
    layout = [[sg.Text('Select Shape:'), sg.Combo(['WURST', 'caWURST', 'supergaussian', 'tanhpulse', 'hspulse', 'loadshape'], key='shape_type')],
              [sg.Text('Select Sequence:'), sg.Combo(['SHAPEsingle', 'double_echo', 'CHORUS', 'compressedCHORUS_cycled', 'double_chirp', 'double_echo_zerophase', 'double_echo_cycled', 'CHORUS_cycled', 'loadshape_double_echo'], key='exp_type')],
...
def read_parameter(exp_type, shape_type):  # Update the "output" text element to be the value of "input" element and set input to parameter
    ...
    if(exp_type == 'CHORUS') or (exp_type == 'CHORUS_cycled') or (exp_type == 'compressedCHORUS_cycled'):  # define CHORUS parameter
